...
This mode targets small, low-risk tasks, so the efficient tier
(`effort: medium` above) is the right default — escalate to the frontier
tier if a task turns out to be larger or riskier than expected mid-run.
`aidw model route <tier>` prints the configured name for either tier (env
`AIDW_FRONTIER_MODEL` / `AIDW_EFFICIENT_MODEL`). Respect an explicit user
model choice without re-prompting, and never claim a model switch happened
unless the host actually performed it.
