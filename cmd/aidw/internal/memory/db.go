...
func (db *DB) init() error {
	_, _ = db.conn.Exec("CREATE TABLE IF NOT EXISTS facts (id INTEGER PRIMARY KEY AUTOINCREMENT, repo_path TEXT, branch TEXT, key TEXT, value TEXT, created_at DATETIME DEFAULT CURRENT_TIMESTAMP, UNIQUE(repo_path, branch, key));")
	
	// Extension loading in mattn/go-sqlite3 requires a build with CGO and specific tags.
	// We'll mark it as disabled for now to allow the facts layer to work.
	db.vectorEnabled = false
	return nil
}

func (db *DB) VectorEnabled() bool { return db.vectorEnabled }
func (db *DB) Status() map[string]any { return map[string]any{"vector_extension_loaded": db.vectorEnabled, "database_connected": db.conn != nil} }
...
func (db *DB) StoreFact(repoPath, branch, key, value string) error {
	_, err := db.conn.Exec("INSERT INTO facts (repo_path, branch, key, value) VALUES (?, ?, ?, ?) ON CONFLICT(repo_path, branch, key) DO UPDATE SET value=excluded.value", repoPath, branch, key, value)
	return err
}
